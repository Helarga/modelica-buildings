...
model BoilerPolynomial_T
  ...
  parameter Modelica.SIunits.HeatFlowRate Q_flow_small = Q_flow_nominal*1E-9
   "Small value for heat flow rate or power, used to avoid division by zero" annotation(Dialog(tab="Advanced"));
  ...
  parameter Real a[6] = {1.124970374,0.014963852,-0.02599835,0.0,-1.40464E-6,-0.00153624}
  ...
  Modelica.SIunits.Efficiency eta=if effCur == Buildings.Fluid.Types.EfficiencyCurves.Constant
       ...
       then Buildings.Utilities.Math.Functions.polynomial(a=a, x=PLRToTset)
  ...
  Modelica.SIunits.Power QFue_flow=PLRToTset*Q_flow_nominal/eta_nominal
  ...
  Modelica.Blocks.Interfaces.RealInput PLRToTset(
    final quantity="ThermodynamicTemperature",
    final unit="K",
    displayUnit="degC",
    min=0) "Set point heating water tempearture."
    annotation (Placement(transformation(extent={{-140,60},{-100,100}})));

  Modelica.Blocks.Interfaces.RealOutput PLR( min=0, max=1)
  ...
  Modelica.Blocks.Interfaces.BooleanInput uMod
    "Control input signal, off=0, heating mode=+1"
...
protected
  ...
  constant Modelica.SIunits.SpecificEnergy h_default=
     Medium.specificEnthalpy_pTX(
       Medium.p_default,
       Medium.T_default,
       Medium.X_default)
  "Default enthalpy when the boiler is off";
  ...
  Modelica.Blocks.Sources.RealExpression Q_flow_set(final
    y=m_flow*(hSet - inStream(port_a.h_outflow)))
    "Required heat flow rate to meet set point"
    annotation (Placement(transformation(extent={{-80,-68},{-60,-48}})));
  Modelica.SIunits.Conversions.NonSIunits.Temperature_degC TBoiLvg_degC
   "Boilerleaving water temperature in degC";
  Modelica.Blocks.Sources.RealExpression TBoiLvg(y=temSen.T)
   "Condenser entering water temperature"
     annotation (Placement(transformation(extent={{-80,-88},{-60,-70}})));
initial equation
...
TBoiLvg_degC =Modelica.SIunits.Conversions.to_degC(TBoiLvg);

boiEffCur = Buildings.Utilities.Math.Functions.biquadratic(xNor, x1=PLR, x2=TBoiLvg_degC);


/*PLR =Buildings.Utilities.Math.Functions.smoothMin(
      x1=QBoi_flow_set/(QBoi_flow_ava - Q_flow_small),
      x2=per.PLRMax,
      deltaX=per.PLRMax/100);*/


Q_flow_ava = boiEffCur*Q_flow_nominal;
 ...
 assert(eta > 0.001, "Efficiency curve is wrong.");
  ...
  connect(vol.heatPort, temSen.port) annotation (Line(
      ...
      color={191,0,0},
      smooth=Smooth.None));
  connect(temSen.T, PLR)
    annotation (Line(points={{20,40},{110,40},{110,80}}, color={0,0,127}));
