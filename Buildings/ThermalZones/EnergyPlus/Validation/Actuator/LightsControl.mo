...
model LightsControl "Validation model for one actuator that controls the lights"
  ...
  Controls.OBC.CDL.Utilities.SunRiseSet sunRiseSet(
    ...
    lon=-1.5344934783534,
    timZon=-21600) "Block that computes sunrise and sunset"
    annotation (Placement(transformation(extent={{-140,130},{-120,150}})));
  Controls.OBC.CDL.Continuous.Modulo mod1 "Output time of day"
    annotation (Placement(transformation(extent={{-100,60},{-80,80}})));
  Controls.OBC.CDL.Continuous.Sources.Constant day(k=24*3600) "Outputs one day"
  ...
  Controls.OBC.CDL.Logical.And and2 "Output true if the lights should be on"
    annotation (Placement(transformation(extent={{20,100},{40,120}})));
  Controls.OBC.CDL.Conversions.BooleanToReal PLig(realTrue=1000)
    "Lighting power"
...
equation
  connect(day.y, mod1.u2) annotation (Line(points={{-128,48},{-120,48},{-120,64},
  ...
  connect(sunRiseSet.nextSunSet, timToSunSet.u1) annotation (Line(points={{-118,
          140},{-110,140},{-110,146},{-102,146}}, color={0,0,127}));
  connect(modTim.y, timToSunSet.u2) annotation (Line(points={{-128,76},{-110,76},
          {-110,134},{-102,134}}, color={0,0,127}));
  connect(lesEquThr1.u, timToSunSet.y)
    annotation (Line(points={{-72,140},{-78,140}}, color={0,0,127}));
  connect(lesEquThr1.y, or2.u1) annotation (Line(points={{-48,140},{-46,140},{-46,
          130},{-42,130}}, color={255,0,255}));
  connect(or2.y, and2.u1) annotation (Line(points={{-18,130},{-14,130},{-14,110},
  ...
  connect(not1.y, or2.u2) annotation (Line(points={{-48,110},{-46,110},{-46,122},
          {-42,122}}, color={255,0,255}));
  connect(not1.u, sunRiseSet.sunUp) annotation (Line(points={{-72,110},{-114,110},
          {-114,134},{-118,134}}, color={255,0,255}));
  connect(greEquThr.u, mod1.y) annotation (Line(points={{-62,40},{-70,40},{-70,70},
          {-78,70}}, color={0,0,127}));
  connect(greEquThr.y, and1.u2) annotation (Line(points={{-38,40},{-34,40},{-34,
          62},{-22,62}}, color={255,0,255}));
  connect(and1.u1, lesEquThr.y)
    annotation (Line(points={{-22,70},{-38,70}}, color={255,0,255}));
  connect(and1.y, and2.u2) annotation (Line(points={{2,70},{10,70},{10,102},{18,
          102}}, color={255,0,255}));
  annotation (
...
First implementation.
    ...
    Diagram(coordinateSystem(extent={{-160,-160},{140,160}})),
