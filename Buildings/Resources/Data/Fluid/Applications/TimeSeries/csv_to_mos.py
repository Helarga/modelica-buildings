...
def timeseries_to_modelica_data(file_path, timestep, data_type, file_name):
# evaluate dimensions of the matrix
    size = timeseries_output.shape
# modifiy the index for modelica mos
    timeseries_output.index = (timeseries_output.index)*timestep
...
timeseries_output = pd.read_csv(os.path.join('EnergyPlus', 'eplusout.csv'))
