...
model TimeSeriesMassFlow_Temp
 ...
 extends Modelica.Icons.Example;
  ...
  parameter String filNam= "modelica://Buildings/Resources/Data/Applications/DHC/Examples/FourthGeneration/massFloRate_Temperature.mos";
  ...
  parameter Modelica.SIunits.MassFlowRate mHW_flow_nominal=
    ...
    annotation (Dialog(group="Design parameter"));
  parameter Modelica.SIunits.TemperatureDifference delTBuiChW(displayUnit=
        "degC")=5
    "Nominal chilled water temperature differnce(building side)";
  parameter Modelica.SIunits.TemperatureDifference delTBuiHW=18
  ...
  EnergyTransferStations.Heating.HeatingIndirect heaETS(
    ...
    mDis_flow_nominal=mHW_flow_nominal,
    dpValve_nominal=6000,
    dp1_nominal=500,
    dp2_nominal=500,
    use_Q_flow_nominal=false,
    Q_flow_nominal=156000,
    T_a1_nominal = 363.15,
    T_a2_nominal = 328.15,
    eta=1,
    reverseActing=true)
  ...
  Fluid.Sources.MassFlowSource_T supDisChWat(
    ...
    nPorts=1) "Chilled water supply district side." annotation (Placement(
        ...
        rotation=0,
        origin={-70,50})));
  Modelica.Blocks.Sources.RealExpression mDisChW(y=buiMasTem.y[2]*(delTBuiChW/
        delTDisChW))
    annotation (Placement(transformation(extent={{-40,70},{-60,90}})));
  Modelica.Blocks.Sources.RealExpression mDisHW(y=buiMasTem.y[1])
    annotation (Placement(transformation(extent={{-40,-8},{-60,12}})));
  Modelica.Blocks.Sources.RealExpression mDisHW1(y=buiMasTem.y[1])
...
equation
  ...
  connect(buiCooSin.ports[1], cooETS.port_b2) annotation (Line(points={{-60,20},
          {-52,20},{-52,22},{-26,22}},
                                     color={0,127,255}));
  connect(buiMasTem.y[2], supChiWat.m_flow_in) annotation (Line(points={{89,10},
          {46,10},{46,30},{32,30}}, color={0,0,127}));
  connect(buiMasTem.y[3], THWR.u) annotation (Line(points={{89,10},{82,10},{82,-50},
          {80,-50}}, color={0,0,127}));
  connect(buiMasTem.y[6], TChWSET.u) annotation (Line(points={{89,10},{82,10},{82,
          80},{2,80}},                 color={0,0,127}));
  connect(buiMasTem.y[4], THWSET.u) annotation (Line(points={{89,10},{82,10},{82,
  ...
  connect(THWR.y, supHeaWat.T_in) annotation (Line(points={{56,-50},{54,-50},{
          54,-66},{36,-66}},
                        color={0,0,127}));
  connect(buiMasTem.y[5], TChWR.u) annotation (Line(points={{89,10},{82,10},{82,
  ...
  connect(mDisHW1.y, supHeaWat.m_flow_in) annotation (Line(points={{55,-24},{46,
          -24},{46,-62},{36,-62}}, color={0,0,127}));
