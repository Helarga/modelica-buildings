...
model HeatingIndirect
  ...
  parameter Modelica.SIunits.Temperature T_a2_nominal
    ...
    annotation(Dialog(group="Heat exchanger"));
      //min=0 + 273,
  parameter Modelica.SIunits.Efficiency eta= 0.8
  ...
  Buildings.Fluid.HeatExchangers.PlateHeatExchangerEffectivenessNTU hex(
    ...
    final T_a2_nominal=T_a2_nominal) "Indirect heatinf heat exchanger"
  ...
  Fluid.Sensors.TemperatureTwoPort senTBuiRet(redeclare final package Medium =
...
equation
  ...
  connect(hex.port_a2, senTBuiRet.port_b) annotation (Line(points={{40,-6},{48,
          -6},{48,-60},{56,-60}}, color={0,127,255}));
...
<p>
Indirect heating energy transfer station (ETS) model that controls 
the building heating water supply temperature by modulating a 
primary control valve on the district supply side. The design is 
based on a typical district heating ETS described in ASHRAE's 
