...
model DistrictCoolingSystemWOHydDia
  "Example to test the district cooling system."
  extends Modelica.Icons.Example;

  package MediumW = Buildings.Media.Water "Medium model for water";
  inner parameter
    Buildings.Applications.DHC.Examples.Combined.Generation5.Unidirectional.Data.DesignDataParallelWOHydDia
    datDes(
    nBui=nBui,
    mDis_flow_nominal=sum({bui_ETS[i].ets.mDis_flow_nominal for i in 1:nBui})*1.01,
    mCon_flow_nominal={bui_ETS[i].ets.mDis_flow_nominal for i in 1:nBui}) "Design data"
  ...
  parameter Modelica.SIunits.Power QEva_nominal=mCHW_flow_nominal*cooPla.numChi*4200*(5 - 14)
  ...
  parameter Modelica.SIunits.Pressure pumDP=dpCHW_nominal+dpSetPoi+400000;
  parameter Modelica.SIunits.Temperature TCHWSet(displayUnit="K")=273.15 + 5
  ...
  Buildings.Applications.DHC.Examples.FifthGeneration.Unidirectional.Networks.UnidirectionalParallel2 disNet(
...
equation
  ...
  connect(TchiWatSet.y, cooPla.TCHWSupSet) annotation (Line(points={{-59,-30},{-50,
          -30},{-50,-33},{-36,-33}}, color={0,0,127}));
...
end DistrictCoolingSystemWOHydDia;
