...
model ChilledWaterPumpSpeed
  ...
  Utilities.Math.SmoothLimit smoothLimit(
    deltaX=0.001,
    upper=1,
    lower=0.0001) "C1 maximum"
...
equation
  ...
  connect(gai.y, conPID.u_m)
    annotation (Line(points={{-55,-40},{-50,-40},{-50,-12}}, color={0,0,127}));
  connect(pumStaCon.y, pumSpe.u1)
    annotation (Line(points={{11,0},{22,0},{22,6},{38,6}}, color={0,0,127}));
  connect(smoothLimit.y, pumSpe[1].u2) annotation (Line(points={{13,-30},{22,
          -30},{22,-6},{38,-6}}, color={0,0,127}));
  connect(smoothLimit.y, pumSpe[2].u2) annotation (Line(points={{13,-30},{22,
          -30},{22,-6},{38,-6}}, color={0,0,127}));
  connect(conPID.y, smoothLimit.u) annotation (Line(points={{-39,0},{-24,0},{
          -24,-30},{-10,-30}}, color={0,0,127}));
