...
partial model PartialPumpParallel "Partial model for pump parallel"
 ...
 // Pump parameters
  ...
  EnergyTransferStations.BaseClasses.HydraulicHeader sucHea(
    redeclare package Medium = Medium,
    m_flow_nominal=m_flow_nominal,
    nPorts_a=1,
    nPorts_b=num) "Suction header."
     annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=180,
        origin={-56,0})));
  EnergyTransferStations.BaseClasses.HydraulicHeader delHea(
   redeclare package Medium = Medium,
    m_flow_nominal=m_flow_nominal,
    nPorts_b=1,
    nPorts_a=num) "Delivery header." annotation (Placement(transformation(
        extent={{10,10},{-10,-10}},
        rotation=180,
        origin={78,0})));
initial equation
...
equation
 for i in 1:num loop
  connect(pum[i].port_b, val[i].port_a)
    annotation (Line(points={{0,0},{28,0}},  color={0,127,255}));
 end for;
  connect(pum.P, P)
    annotation (Line(points={{1,9},{20,9},{20,40},{110,40}},
      color={0,0,127}));
  connect(booToRea.y, val.y)
    annotation (Line(points={{2,-40},{38,-40},{38,-12}},
                                                       color={0,0,127}));
  connect(hys.y, booToRea.u)
    annotation (Line(points={{-40,-40},{-22,-40}},
                                              color={255,0,255}));
  connect(hys.u, u) annotation (Line(points={{-64,-40},{-122,-40}},
        color={0,0,127}));
  connect(port_a,sucHea. ports_a[1]) annotation (Line(points={{-100,0},{-66,0},
          {-66,8.88178e-16}},color={0,127,255}));
  connect(sucHea.ports_b, pum.port_a) annotation (Line(points={{-46,
          -1.33227e-15},{-34,-1.33227e-15},{-34,0},{-20,0}},
                                               color={0,127,255}));
  connect(delHea.ports_b[1], port_b) annotation (Line(points={{88,-1.11022e-15},
          {86,-1.11022e-15},{86,0},{100,0}}, color={0,127,255}));
  connect(val.port_b, delHea.ports_a)
    annotation (Line(points={{48,0},{68,0}}, color={0,127,255}));
