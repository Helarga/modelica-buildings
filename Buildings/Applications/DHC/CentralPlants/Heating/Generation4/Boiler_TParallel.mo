...
model Boiler_TParallel "Multiple identical boiler"
  extends
    Buildings.Applications.DHC.CentralPlants.Heating.Generation4.PartialPlantParallel(
     redeclare Buildings.Fluid.HeatExchangers.Heater_T boi(
  ...
  Modelica.Blocks.Interfaces.RealOutput Q_Boi[numBoi] "Boiler heat flow rate."
    annotation (Placement(transformation(extent={{100,40},{120,60}}),
        iconTransformation(extent={{100,40},{120,60}})));
  Modelica.Blocks.Math.BooleanToReal booToRea[numBoi](each final realTrue=1, each final
            realFalse=0)
